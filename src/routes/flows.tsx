...
export const Route = createFileRoute("/flows")({ component: Page });

function Page() {
  const total = flows.length;
  const synced = flows.filter((q) => q.status === "synced").length;
  const missing = flows.filter((q) => q.status === "missing").length;
  const drift = flows.filter((q) => q.status === "drift").length;
      ...
      <div className="max-w-[1500px] mx-auto space-y-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-3">
          <div>
            ...
            <h1 className="text-3xl font-bold tracking-tight mt-1">Flows</h1>
        ...
        <ObjectTable data={flows} kind="Flows" />
      </div>
    </AppShell>
  );
}
