...
export const Route = createFileRoute("/divisions")({ component: Page });

function Page() {
  const total = divisions.length;
  const synced = divisions.filter((q) => q.status === "synced").length;
  const missing = divisions.filter((q) => q.status === "missing").length;
  const drift = divisions.filter((q) => q.status === "drift").length;
      ...
      <div className="max-w-[1500px] mx-auto space-y-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-3">
          <div>
            ...
            <h1 className="text-3xl font-bold tracking-tight mt-1">Divisions</h1>
        ...
        <ObjectTable data={divisions} kind="Divisions" />
      </div>
    </AppShell>
  );
}
